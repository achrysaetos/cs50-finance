...
exports.quote_post = (req, res) => {
    if (!req.session.userID)
        res.redirect("login");
    try {
        var url = `https://cloud.iexapis.com/stable/stock/${req.body.quotesymbol}/quote?token=${api_key}`;
    ...
    try {
        ...
        try {
            ...
            for (stocks of stocksarray) {
                var url = `https://cloud.iexapis.com/stable/stock/${stocks.symbol}/quote?token=${api_key}`;
                var xhr = new XMLHttpRequest();
                xhr.open("GET", url, false);
                xhr.send(null);
